...
const Autocomplete2 = () => {
  ...
  const fetchSuggestions = async (input) => {
    ...
    try {
      const response = await fetch(`/api/v2/autocomplete?query=${input}`, {
        mode: "cors",
        headers: {
  ...
  return (
    <div className="p-6 w-full max-w-md mx-auto bg-white rounded-lg shadow-lg">
      <h1 className="text-2xl font-semibold text-center text-gray-700 mb-4">
        Autocomplete Search (v2)
      </h1>

      <div className="text-gray-600 text-sm text-center mb-2">
        <p>No. of searches: <span className="font-bold text-blue-600">{countv2}</span></p>
